  ...
  return (
    <div className='flex items-center justify-between'>
      <div className='flex flex-1 flex-col-reverse items-start gap-y-2 sm:flex-row sm:items-center sm:space-x-2'>
        <Input
          placeholder='Xidmət ilə axtarış...'
          value={
        ...
        />
        <Input
          placeholder='Tarix ilə axtarış...'
          value={
            (table.getColumn('changeDate')?.getFilterValue() as string) ?? ''
          }
          onChange={(event) =>
            table.getColumn('changeDate')?.setFilterValue(event.target.value)
          }
          className='h-8 w-[150px] lg:w-[250px]'
        />
        {isFiltered && (
